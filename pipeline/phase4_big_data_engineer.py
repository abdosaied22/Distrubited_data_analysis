...
spark = (
    SparkSession.builder.appName("Phase4_BigDataEngineer_Amazon")
...
df_raw = (
    spark.read.option("header", "true").option("inferSchema", "true").csv(CLEANED_INPUT)
)
...
# --- 4. Skew Detection ---
skew_data = (
    df_raw.groupBy("Category").count().orderBy(F.desc("count")).limit(5).collect()
)
master_metrics["skew_report"] = [
    {"category": r["Category"], "count": r["count"]} for r in skew_data
]

# --- 5. Broadcast Join ---
state_meta = df_raw.select("State").distinct().withColumn("Region", F.lit("Standard"))
timed_log(
    "Join", "before", "shuffle_join", lambda: df_raw.join(state_meta, "State").count()
)
timed_log(
    "Join",
    "after",
    "broadcast_join",
    lambda: df_raw.join(F.broadcast(state_meta), "State").count(),
)

# --- 6. Final Aggregation & Output ---
output_csv_path = os.path.join(RESULTS_DIR, "final_optimized_data_csv")
optimized_agg = df_optimized.groupBy("State", "Category").agg(
...
timed_log(
    ...
    "after",
    "optimized_csv_write",
    lambda: optimized_agg.coalesce(1)
